...
const ProjectCard = ({ project, onClick }) => {
  ...
  const isVideoMedia = primaryMedia.type === 'video' || isVideo(primaryMedia);
        ...
        >
          {!mediaLoadError && isVideoMedia ? (
            <VideoPlayer
              ...
              loop={false}
              poster={primaryMedia.poster ? `${import.meta.env.BASE_URL || ''}${primaryMedia.poster}` : `${import.meta.env.BASE_URL || ''}/assets/images/placeholders/project.jpg`}
              onError={handleMediaError}
            />
          ) : (
            <Box
              component="img"
              src={mediaLoadError ? `${import.meta.env.BASE_URL || ''}/assets/images/placeholders/project.jpg` : mediaSource}
              alt={`${title} preview`}
              sx={{
                width: '100%',
                height: '100%',
