...
const ThemeDebugger = () => {
    ...
    <Box>
      {[0, 0.5, 1, 2, 3, 4, 6, 8].map((factor) => (
        <Box key={factor} sx={{ display: 'flex', alignItems: 'center', mb: 1 }}>
          ...
          <Typography variant="body2">
            theme.spacing({factor}) = {theme.spacing(factor)}
          </Typography>
          <IconButton
            size="small"
            onClick={() => copyToClipboard(theme.spacing(factor))}
  ...
  // Animations preview
  const AnimationsPreview = () => <Box>{renderObject(theme.transitions)}</Box>;
