...
const TorusScene = ({
  ...
  const getActiveTrailColor = useCallback(() => {
    if (easterEggActive) {
      ...
      const themeHsl = rgbToHsl(primaryColor.r, primaryColor.g, primaryColor.b);
      ...
      return new THREE.Color().setHSL(animatedHue, hsl.s, hsl.l);
    }
  ...
  const [pointsPool] = useState(
    () =>
      new ObjectPool(
        ...
        40
      ) // Initialize with enough points to avoid reallocations
  );
  ...
  const releaseTrailPoint = useCallback(
    (point) => {
  ...
  useEffect(() => {
    if (mousePosition) {
      cursorPosition.current.set(mousePosition.x * 4, mousePosition.y * 2, 0);
  ...
  useFrame(() => {
    ...
    if (frameCounter.current % 5 === 0) {
      // Changed from 15 to 5 for more frequent updates
      ...
      const shouldAddPoint =
        !lastPoint ||
        newPosition.distanceTo(new THREE.Vector3(lastPoint.position.x, lastPoint.position.y, 0)) >
          0.2;
  ...
  useEffect(() => {
    if (easterEggActive && isMounted.current) {
      ...
      for (let i = 0; i < burstCount; i++) {
        ...
        const point = getTrailPoint();
        point.position.set(Math.cos(angle) * radius, Math.sin(angle) * radius, 0);
        point.time = clock.getElapsedTime();
        point.size = 0.3;
  ...
  useFrame(() => {
    ...
    if (frameCounter.current % 2 === 0 && shouldProcessInteraction) {
      ...
      const mouseSpeed = mousePosition
        ? new THREE.Vector2(mousePosition.x, mousePosition.y).distanceTo(prevMousePos.current)
        : 0;
      ...
      const shouldAddToTrail =
        ...
        mouseSpeed > 0.005 ||
        cursorPosition.current.distanceTo(
          new THREE.Vector3(
  ...
  useEffect(() => {
    if (mousePosition && cursorTrail.length > 0) {
      ...
      const lastPoint = autoTrailPoints[autoTrailPoints.length - 1];
      trailPositionRef.current = [lastPoint?.position?.x || 0, lastPoint?.position?.y || 0, 0];
    }
    ...
    <group>
      ...
      {activeTrail.map((point, i) => {
        ...
        if (easterEggActive) {
          ...
          if (specialEffect) {
            return (
              <group key={`special-${i}`} position={point.position} rotation={point.rotation}>
                <mesh>
                ...
                <pointLight intensity={0.5} distance={3} color={new THREE.Color(1, 0.3, 0.8)} />
              </group>
            );
          }
        ...
        const velocityThickness =
          hasMouseMoved.current && directionIntensity.current > 0.2
            ? thickness * (1 + directionIntensity.current * 0.5)
            : thickness;
              ...
              <mesh>
                ...
                <meshStandardMaterial
                  color={new THREE.Color().setHSL(
                    (i / activeTrail.length + clock.getElapsedTime() * 0.1) % 1,
                    0.8,
                    0.6
                  )}
                  ...
                  emissive={new THREE.Color().setHSL(
                    (i / activeTrail.length + 0.5 + clock.getElapsedTime() * 0.1) % 1,
                    0.8,
                    0.6
                  )}
            ...
            >
              ...
              <mesh>
                ...
                <meshStandardMaterial
                  ...
                  transparent={true}
                  emissive={
                    directionIntensity.current > 0.3
                      ? new THREE.Color().setHSL((directionHue.current + 0.1) % 1.0, 0.8, 0.4)
