 ...
 */
const InteractionGuide = ({ show = true, autoHideDelay = 5000, position = 'bottom' }) => {
  ...
  const getInstructions = () => {
    switch (currentShapeType) {
      ...
      default:
        return {
          action: isTouchDevice ? 'Tap to change scene' : 'Click to change scene',
          secondary: isTouchDevice ? 'Touch and drag to rotate view' : 'Drag to rotate view',
        };
    }
  };
        ...
        {isTouchDevice ? (
          <>
            <TouchAppIcon
              ...
              sx={isMobile ? { fontSize: 18 } : {}}
            />
            <Box>
              <Typography
                variant="body2"
                fontWeight="bold"
                sx={{ fontSize: isMobile ? '0.49rem' : '1rem' }}
              >
                {instructions.action}
              </Typography>
              <Typography
                variant="body2"
                sx={{ opacity: 0.8, mt: 0.5, fontSize: isMobile ? '0.46rem' : '1rem' }}
              >
