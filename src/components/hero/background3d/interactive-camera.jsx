...
const InteractiveCamera = ({ enableAutoRotate = true, rotateSpeed = 0.2 }) => {
  ...
  useEffect(() => {
    ...
    function updateTarget() {
      ...
      const easeProgress =
        progress < 0.5 ? 2 * progress * progress : 1 - Math.pow(-2 * progress + 2, 2) / 2; // Ease in-out quad

      const newTarget = new THREE.Vector3().lerpVectors(startTarget, target, easeProgress);
  ...
  return (
    <>
      <PerspectiveCamera makeDefault fov={40} near={0.1} far={100} />
